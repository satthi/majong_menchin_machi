...
public class Majong {
    ...
    public static void main(String[] args) {
        ...
        int correct_count = 0;
        for (int q = 1; q <= 5; q++){
            System.out.println("第" + q + "問");
            if (question() == true){
                correct_count++;
            }
        }
        
        System.out.println("正解は" + correct_count + "問");
    ...
    public static Boolean question() {
        ...
        */
        ...
        ArrayList<Integer> agari_numbers = new ArrayList<Integer>();
        
        if (chitoi_shanten > 0 && mentsh_shanten > 0){
            //ノーテン
            //System.out.println("ノーテン");
        } else {
            //System.out.println("テンパイ");
            int before_hai_num = 0;
            for (int check_num = 1; check_num <= 36; check_num++) {
                if (hais.contains(check_num)){
                    continue;
                }
                Map<String,Integer> hai_info = haipai.get(check_num);
                if (
                ...
                ArrayList<Integer> check_hais = new ArrayList<Integer>(hais);
                check_hais.add(check_num);
                Collections.sort(check_hais);
                if (agariCheck(haipai, check_hais) == true){
                    agari_numbers.add(hai_info.get("num"));
                }
                before_hai_num = check_num;
            }
        }
        
        // 標準入力を設定 
        System.out.println("回答を半角数字で入力してください。ノーテンの場合は空で進んでください。");
        Boolean correct_flag = true;
            
        try { 
            BufferedReader bin = new BufferedReader(new InputStreamReader(System.in)); 
            String answer = bin.readLine(); 
            
            String[] answer_numbers = answer.split("");
            for (int i = 1; i <= 9; i++){
                if (agari_numbers.contains(i)){
                    if (!Arrays.asList(answer_numbers).contains(String.valueOf(i))){
                        correct_flag = false;
                    }
                } else {
                    if (Arrays.asList(answer_numbers).contains(String.valueOf(i))){
            ...
            if (correct_flag == true){
                ...
                System.out.println("チガーウ");
            }
            
            System.out.println(answer);
            System.out.println(agari_numbers);
